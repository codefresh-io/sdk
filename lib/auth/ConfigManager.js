...
class ConfigManager {
    ...
    isConfigLoaded() {
        return !_.isEmpty(this.currentContextName);
    }
    ...
    async loadConfig(configFilePath = defaults.CF_CONFIG_PATH) {
        ...
        if (this.isConfigLoaded() && configFilePath === this._getConfigFilePath()) {
        ...
        _.forEach(config.contexts, this._parseContext.bind(this));
