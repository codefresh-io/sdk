...
function _processApi(swagger) {
    return _.reduce(swagger.apis, (acc, resource, resourceName) => {
        const metKeys = new Set();
        _.forEach(resource, (method, key) => {
            const noNumberKey = key.replace(/\d+$/, '');
            if (noNumberKey !== key && !metKeys.has(noNumberKey)) {
                delete resource[key]; // eslint-disable-line no-param-reassign
                resource[noNumberKey] = method; // eslint-disable-line no-param-reassign
                metKeys.add(noNumberKey);
            }
        });
        const temp = {};
        _.set(temp, resourceName, resource);
