...
const { postProcessApi } = require('../helpers');

const DEFAULT_CODEFRESH_URL = 'https://g.codefresh.io';
const DEFAULT_SPEC_URL = `${DEFAULT_CODEFRESH_URL}/api/openapi.json`;
...
class Client {
    constructor(config) {
        if (config && !config.apiKey) {
            console.warn('Warning: api key for codefresh-sdk is not provided - only non-authenticated requests available');
        }
        this.config = config;
        this.api = null;
    }
    ...
    async loadClient() {
        if (!this.api) {
            ...
            const { specUrl, apiKey, url } = this.config;
            ...
            if (!spec) {
                const response = await request({ url: specUrl || DEFAULT_SPEC_URL, method: 'GET' });
                this.handleErrors(response);
