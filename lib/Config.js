...
class Config {
    ...
    static async fromFile(options = {}) {
        ...
        if (!manager.hasContexts()) {
            debug('config file has no contexts - using NoAuthConfig');
            const noAuthContext = new NoAuthContext({ url: defaults.URL });
    ...
    static async _initializeConfig(context, options = {}) {
        ...
        request.baseUrl = context.url; // for non-swagger api calls
